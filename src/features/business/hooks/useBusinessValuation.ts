...
export const useBusinessValuation = (): UseBusinessValuationReturn => {
  ...
  const calculateConfidenceLevel = useCallback((inputs: ValuationInputs): 'high' | 'medium' | 'low' => {
    let score = 0;
  ...
  }, [inputs, calculateDCF, calculateMarketMultiple, calculateAssetBased, calculateRiskAdjustment, calculateConfidenceLevel]);
  ...
  // Save valuation
  const saveValuation = useCallback(async (name: string): Promise<string> => {
  ...
  // Export valuation
  const exportValuation = useCallback(async (format: 'pdf' | 'excel'): Promise<void> => {
