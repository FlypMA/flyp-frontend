...
interface ValuationModalProps {
  ...
  onListingNudge?: (valuationData: ValuationInputs, businessValue: number) => void; // For listing nudge
}
...
const ValuationModal: React.FC<ValuationModalProps> = ({
  ...
  const calculateValuation = async () => {
    ...
    try {
      ...
      // For authenticated users, show listing nudge if available, otherwise save and navigate
      if (isAuthenticated) {
        if (onListingNudge) {
          // Show the gentle listing nudge with valuation results
          onListingNudge(inputs, finalValuation);
        } else if (onComplete) {
          // Fallback: save the valuation and navigate to valuations page
          onComplete(inputs);
