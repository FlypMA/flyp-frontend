...
const MyBusinesses: React.FC<MyBusinessesProps> = () => {
  ...
  const getROI = (business: OwnedBusiness) => {
    ...
    const yearsOwned = (new Date().getTime() - new Date(business.acquisition_date).getTime()) / (1000 * 60 * 60 * 24 * 365);
  ...
  const totalPortfolioValue = businesses.reduce((sum, business) => 
    sum + (business.current_valuation || business.acquisition_price), 0
  );

  const totalInvestment = businesses.reduce((sum, business) => 
    sum + business.acquisition_price, 0
  );
    ...
    <div className="space-y-6">
        ...
        <Card className="border border-gray-200">
          <CardBody className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                <p className="text-2xl font-bold text-gray-900">{formatPrice(totalPortfolioValue)}</p>
      ...
      <div className="flex items-center gap-2 border-b border-gray-200">
        {[
          ...
          { key: 'operating', label: 'Operating', count: businesses.filter(b => b.status === 'operating').length },
          { key: 'for_sale', label: 'For Sale', count: businesses.filter(b => b.status === 'for_sale').length },
      ...
      {filteredBusinesses.length > 0 ? (
        <div className="space-y-6">
          {filteredBusinesses.map(business => {
            ...
            return (
              <Card key={business.id} className="border border-gray-200 hover:border-primary-300 hover:shadow-lg transition-all duration-200">
                <CardBody className="p-6">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-600">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span>{business.region}, {business.country}</span>
                            </div>
                            <div className="flex items-center gap-1">
                          ...
                          <Dropdown>
                            ...
                            <DropdownMenu>
                              {business.status === 'operating' && (
                                <>
                                  <DropdownItem key="sell" startContent={<Target className="w-4 h-4" />} onPress={() => createListing(business)}>
                                    List for Sale
                                  </DropdownItem>
                                  <DropdownItem key="valuation" startContent={<Calculator className="w-4 h-4" />} onPress={() => getValuation(business)}>
                                    Get Valuation
                                  </DropdownItem>
                                </>
                              )}
                              {business.listing_id && (
                                <DropdownItem key="view-listing" startContent={<Eye className="w-4 h-4" />} onPress={() => navigate(`/listings/${business.listing_id}`)}>
                                  View Listing
                                </DropdownItem>
                              )}
                              ...
                              <DropdownItem key="reports" startContent={<FileText className="w-4 h-4" />}>
                                View Reports
                              </DropdownItem>
                            </DropdownMenu>
                          </Dropdown>
                          ...
                          <>
                            <div className="text-center p-4 bg-purple-50 rounded-lg border border-purple-200">
                              ...
                              <p className="text-xl font-bold text-purple-900">
                                {roi.totalReturn > 0 ? '+' : ''}{formatPrice(roi.totalReturn, business.currency)}
                              </p>
                            ...
                            <div className="text-center p-4 bg-orange-50 rounded-lg border border-orange-200">
                              ...
                              <p className="text-xl font-bold text-orange-900">
                                {roi.annualizedROI > 0 ? '+' : ''}{roi.annualizedROI.toFixed(1)}%
                              </p>
                      ...
                      {(business.annual_revenue || business.annual_ebitda) && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          ...
                          {business.annual_ebitda && (
                            <div>
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-gray-700">Annual EBITDA</span>
                                <span className="text-sm font-bold text-gray-900">
          ...
          <CardBody className="text-center py-16">
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-4">
                {filter === 'all' ? 'No businesses owned yet' : `No ${filter.replace('_', ' ')} businesses`}
              </h3>
          ...
          <CardBody className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-600">
                  You've successfully acquired and grown businesses. Consider selling one to realize your returns.
                </p>
              </div>
              <Button
                color="primary"
